...
export default class WebViewBridge extends EventEmitter implements IBridgeConstructorOptions {
  /**
  ...
  private initWebView() {
    this.webView.onChangedURL = (event) => {
      if (event.url.startsWith(this.scheme + "://")) {
        ...
        try {
          eventObject = JSON.parse(decodeURIComponent(event.url.replace(this.scheme + "://", "")));
        } catch (ex) {}
  ...
  refresh(): void {
    ...
    if (isFile) {
      this.webView.loadFile(typeof this.source === "string" ? new File({ path: this.source }) : this.source);
    } else {
